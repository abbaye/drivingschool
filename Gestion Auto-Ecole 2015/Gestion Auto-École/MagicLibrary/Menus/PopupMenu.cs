	...
	//						Console.WriteLine("Track {0} {1}", this.Handle, ((Win32.Msgs)msg.message).ToString());
        ...
        protected void SetWindowRegion(Size winSize)
        {
            ...
            if (_style == VisualStyle.IDE)
            {
                ...
                if (!((_direction == DirectionUL.Vertical) && !_excludeTop))
                {
		...
		protected void DefineColors(Color backColor)
		{
			...
			_controlLL = ColorHelper.CalculateColor(SystemColors.Window, _backColor, 220);
			_controlLBrush = new SolidBrush(ColorHelper.CalculateColor(_backColor, Color.White, 200));
			_controlEBrush = new SolidBrush(ColorHelper.CalculateColor(_backColor, Color.White, 150));
