...
namespace Crownwood.Magic.Docking
{
    ...
    public class DockingManager
    {
                                                     ...
                                                     ContextHandler showContextMenu)
        {
            ...
            WindowDetailCaption wdc = _visualStyle == VisualStyle.IDE
                ? new WindowDetailCaptionIDE(this, contentClose, restore,
                                                 invertAutoHide, showContextMenu)
                : (WindowDetailCaption)new WindowDetailCaptionPlain(this, contentClose, restore,
                                                   invertAutoHide, showContextMenu);
        ...
        public void ValuesFromState(State newState, out DockStyle dockState, out DirectionUL direction)
        {
            switch(newState)
            {
                case State.Floating:
        ...
        internal AutoHidePanel AutoHidePanelForContent(Content c)
        {
            ...
            return _ahpBottom.ContainsContent(c) ? _ahpBottom : null;
        }
		    ...
		    if (_autoResize)
		    {
			    ...
			    if ((f == null) || ((f != null) && (f.WindowState != FormWindowState.Minimized)))
			    {
				    if ((inner.Width < 0) || (inner.Height < 0))
				    {
					    ...
					    if (inner.Width < 0)
							ResizeDirection(-inner.Width, zcLeft, zcRight, DirectionUL.Horizontal);

						if (inner.Height < 0)
							ResizeDirection(-inner.Height, zcTop, zcBottom, DirectionUL.Vertical);
		...
		protected void ResizeDirection(int remainder, ZoneCollection zcAlpha, ZoneCollection zcBeta, DirectionUL dir)
		{
			bool alter;
			int available;
			int half1, half2;
			...
			while((remainder > 0) && ((zcAlpha.Count > 0) || (zcBeta.Count > 0)))
			{
				...
				if (zcAlpha.Count > 0)
				{
					...
					available = dir == DirectionUL.Horizontal ? z.Width - z.MinimumWidth : z.Height - z.MinimumHeight;
